...
data/temp/captured_faces → 학습 → 새로운 모델 구축 및 임베딩 → 인식
"""
...
class ModelTrainingPipeline:
    ...
    def __init__(self):
        """초기화"""
        # 데이터 경로
        self.captured_faces_dir = Path("data/temp/captured_faces")
    ...
    def _prepare_and_validate_data(self) -> Dict:
        ...
        if not self.captured_faces_dir.exists():
            return {'is_valid': False, 'error': 'captured_faces 폴더가 없습니다.'}
        
        # 얼굴 파일들 수집
        face_files = list(self.captured_faces_dir.glob("*.jpg"))
        metadata_files = list(self.captured_faces_dir.glob("*.json"))
