...
class FaceDetector:
    ...
    def _initialize_model(self):
        ...
        if self.model_type in ['haar', 'haar_default']:
            ...
            try:
                # RetinaFace 모델 초기화 (실제 구현 필요)
                # 현재는 구현되지 않았으므로 Haar로 폴백
                self.logger.warning(f"RetinaFace 모델은 아직 구현되지 않음: {self.model_config['name']}")
                self._fallback_to_default_haar()
...
class EnhancedFaceCaptureSystem:
    ...
    def _draw_info_overlay(self, frame: np.ndarray, detections: List[Dict]) -> np.ndarray:
        ...
        # 텍스트 정보
        texts = [
            f"Camera: {self.camera_id} | Model: {self.current_model['name'] if self.current_model else 'None'}",
            f"Mode: {'AUTO' if self.is_auto_mode else 'MANUAL'} | FPS: {self.current_fps:.1f}",
            f"Faces: {len(detections)} | Auto-Save: {'ON' if self.auto_save_enabled else 'OFF'}",
            f"Model Settings - Conf: {self.face_detector.confidence_threshold:.2f}, Scale: {self.face_detector.scale_factor:.2f}",
            f"Session Stats - Auto: {self.session_stats['auto_saved']}, Manual: {self.session_stats['manual_captured']}"
        ]
